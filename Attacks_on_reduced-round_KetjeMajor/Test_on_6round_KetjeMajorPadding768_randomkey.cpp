/* 
2 	Test_on_6round_KetjeMajorPadding768_randomkey.cpp 
3 	Test whether the cube sum with a random key guess(wrong key guess in common cases) is zero or not. 
...
int main()
{
	...
	for(j=0;j<(UINT64(1)<<32);j++)//initialize tempstate
	{
		...
		for(k=0;k<2;k++){
			if(temp1){
				TempState[index0[0][k]]|=UINT64(1)<<index0[1][k];
			}
			else{
				TempState[index0[0][k]]&=(~(UINT64(1)<<index0[1][k]));
			}
