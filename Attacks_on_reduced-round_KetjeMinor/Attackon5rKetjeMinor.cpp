/*
Attackon5rKetjeMinor.cpp;
The program is run in Visual Studio 2012 with x64 platform Release;
The time is about 6 minutes for one recovery of 12 key bits.
*/
...
int main()
{
	...
	for(i=0;i<(1<<12);i++)
	{
		...
		temp1=(tempkey[8]&1) ^ ((InitialState[3]>>16)&1) ^ ((InitialState[8]>>16)&1) ^ ((InitialState[10]>>15)&1) ^ ((InitialState[13]>>16)&1) ^ ((InitialState[14]>>16)&1) ^ ((InitialState[15]>>15)&1) ^ ((InitialState[20]>>15)&1) ^ ((InitialState[23]>>16)&1);
		if(temp!=temp1)
		{
		...
		for(j=0;j<(INT64(1)<<16);j++)//initialize tempstate
		{
		...
		{
			GIndex[counter]=counter;
			for(j=0;j<12;j++)
			{
				guessedkey[counter][j]=tempkey[j];
			}
			counter++;
		}
	}
	...
	for(i=0;i<40;i++)
	{
		if(GIndex[i]!=-1)
		{
			...
			for(j=0;j<12;j++)
			{
				fprintf(f,"%d",guessedkey[i][j]);
				printf("%d",guessedkey[i][j]);
			}
			printf("\n");
		}
